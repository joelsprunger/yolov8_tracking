...
def run(
    ...
    filter_classes = [0, 1]  #person, bicycles
    # Create trackers for each video, and each counting class
    trackers: list[dict] = []
    for i in range(bs):
        temp_dict = {}
        for c in filter_classes:
            ...
            temp_dict[c] = tracker
            if hasattr(temp_dict[c], 'model'):
                if hasattr(temp_dict[c].model, 'warmup'):
                    temp_dict[c].model.warmup()
        trackers.append(temp_dict)
    ...
    for frame_idx, batch in enumerate(dataset):
        ...
        with dt[2]:
            ...
            for i, det in enumerate(p):  # detections per image
                ...
                for c in filter_classes:
                    if hasattr(trackers[i][c], 'tracker') and hasattr(trackers[i][c].tracker, 'camera_update'):
                        if prev_frames[i] is not None and curr_frames[i] is not None:  # camera motion compensation
                            trackers[i][c].tracker.camera_update(prev_frames[i], curr_frames[i])
                ...
                if det is not None and len(det):
                    ...
                    for c in filter_classes:
                        with dt[3]:
                            outputs[i][c] = trackers[i][c].update(det[det[:, 5] == c].cpu(), im0)
                    ...
                    if len(outputs[i]) > 0:
                        ...
                        for c in filter_classes:
                            for j, (output) in enumerate(outputs[i][c]):
                                bbox = output[0:4]
                                id = output[4]
                                cls = output[5]
                                conf = output[6]
                                ...
                                if save_vid or save_crop or show_vid:  # Add bbox/seg to image
                                    ...
                                    if save_trajectories and tracking_method == 'strongsort':
                                        q = output[7]
                                        trackers[i][c].trajectory(im0, q, color=color)
                                    if save_crop:
